...
class Users(object):
    ...
    def getIDByEmployeeID(self, server, token, employeeID):
        """[summary]
        ...
        Returns:
            [type] -- [description]
        """
        self.uri = '/api/v1/users?employee_num={0}'.format(employeeID)
        self.server = server + self.uri 
        headers = {'Content-Type': 'application/json','Authorization': 'Bearer {0}'.format(token)}
        results = requests.get(self.server, headers=headers)
        jsonData = json.loads(results.content)
    ...
    def getFullNameByEmployeeID(self, server, token, employeeID):
        """[summary]
