...
class Assets(object):
    ...
    def create(self, server, token, asset_tag, status_id, model_id, name):
        """Create new asset data
        
        Arguments:
            server {string} -- Server URI
            token {string} -- Token value to be used for accessing the API
            "status_id" {string} -- Status ID of the asset
            "name" {string} -- Name of the asset
            "asset_tag" {string} -- Asset tag of the asset
            "model_id" {string} -- Model ID of the asset
    ...
    def checkOutAsset(self, server, token, assetID, userID, note=None, locationID=None):
        """Check out an asset
        ...
        Arguments:
            ...
            token {string} -- Token value to be used for accessing the API
            assetID {string} -- ID of the asset
            userID {string} -- ID of the user to whom the asset is checked out
    ...
    def getAssetNameByTag(self, server, token, assetTag):
        """Get list of assets based on search keyword
        
        Arguments:
            server {string} -- Server URI
            token {string} -- Token value to be used for accessing the API
            assetTag {string} -- Asset Tag to find
        
        Returns:
            string -- List of assets in JSON format.
        """
        
        self.uri = '/api/v1/hardware/bytag/asset_tag?{0}'.format(assetTag)
        self.server = server + self.uri
        headers = {'content-type': 'application/json','Authorization': 'Bearer {0}'.format(token)}
        results = requests.get(self.server, headers=headers)
        
    
    def getDataBySerial(self, server, token, AssetSerial):
        """Get asset details by Serial Number
        ...
        jsonData = json.loads(results.content)
        if len(jsonData['rows']) < 2 and jsonData['rows'][0]['id'] is not None:
            AssetName = jsonData['rows'][0]['name']
            AssetTag = jsonData['rows'][0]['id']
        return AssetName, AssetTag
